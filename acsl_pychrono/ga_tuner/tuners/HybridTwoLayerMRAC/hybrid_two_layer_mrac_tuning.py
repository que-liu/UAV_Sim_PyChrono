"""
HybridTwoLayerMRAC controller parameter tuning implementation.

This module defines parameter bounds, encoding/decoding for HybridTwoLayerMRAC controller gains.
HybridTwoLayerMRAC extends TwoLayerMRAC with additional hybrid-switching parameters.
"""
...
class HybridTwoLayerMRACTuning(ControllerTuningInterface):
    """
    HybridTwoLayerMRAC controller tuning interface.
    
    Handles:
    - Parameter bounds for Gamma matrices, Q matrices, reference model gains
    - Two-layer specific parameters (second layer adaptive rates Gamma_g)
    - Hybrid-switching parameters (alpha_hybrid, tolerance_time_reset)
    - Encoding/decoding between parameter vectors and gain structures
    - Cholesky parameterization for positive-definite matrices
    
    Note: HybridTwoLayerMRAC has same structure as TwoLayerMRAC plus hybrid parameters.
    """
    
    def __init__(self, tuning_config: Dict[str, Any] = None):
        """
        Initialize HybridTwoLayerMRAC tuning interface.
        
        Args:
            tuning_config: Optional configuration (unused, kept for interface compatibility)
        """
        # Load baseline gains from HybridTwoLayerMRAC implementation
        self._baseline_gains = self._load_default_hybrid_two_layer_mrac_gains()

        # Define parameter bounds using same approach as TwoLayerMRAC
        ...
        for name in ['sigma_x_tran', 'sigma_r_tran', 'sigma_Theta_tran', 'sigma_g_tran',
                     'sigma_x_rot', 'sigma_r_rot', 'sigma_Theta_rot', 'sigma_g_rot']:
            base_value = float(self._baseline_gains.get(name, 0.5))
            self.bounds[name] = _positive_bounds(base_value, factor=3.0,
                                                 min_lower=0.05, max_upper=1.5)

        # Hybrid-switching parameters (if present)
        for name in ['alpha_hybrid_series_tran', 'alpha_hybrid_series_rot']:
            if name in self._baseline_gains:
                base_value = float(self._baseline_gains[name])
                self.bounds[name] = [max(1.0001, base_value / 2.0), base_value * 2.0]
        
        for name in ['tolerance_time_reset_series_hybrid_tran', 'tolerance_time_reset_series_hybrid_rot']:
            if name in self._baseline_gains:
                base_value = float(self._baseline_gains[name])
                self.bounds[name] = _positive_bounds(base_value, factor=10.0,
                                                     min_lower=1e-3, max_upper=1.0)
    ...
    def _load_default_hybrid_two_layer_mrac_gains(self) -> Dict[str, Any]:
        """Load baseline gains from HybridTwoLayerMRAC implementation."""
        from acsl_pychrono.control.HybridTwoLayerMRAC import HybridTwoLayerMRACGains
        from acsl_pychrono.simulation.flight_params import FlightParams
        
        # Create default flight params
        flight_params = FlightParams()
        gains_obj = HybridTwoLayerMRACGains(flight_params)
        
        # Extract relevant gains
        defaults = {
            # Translational Gamma matrices (first layer)
            ...
            # Rotational Gamma matrices (first layer)
            ...
            # Second layer Gamma matrices
            'Gamma_g_tran': np.asarray(gains_obj.Gamma_g_tran),
            'Gamma_g_rot': np.asarray(gains_obj.Gamma_g_rot),
        ...
        # Add e-modification parameters
        for param in ['sigma_x_tran', 'sigma_r_tran', 'sigma_Theta_tran', 'sigma_g_tran',
                      'sigma_x_rot', 'sigma_r_rot', 'sigma_Theta_rot', 'sigma_g_rot']:
            if hasattr(gains_obj, param):
                defaults[param] = float(getattr(gains_obj, param))
        
        # Add hybrid-switching parameters
        for param in ['alpha_hybrid_series_tran', 'alpha_hybrid_series_rot',
                      'tolerance_time_reset_series_hybrid_tran', 'tolerance_time_reset_series_hybrid_rot']:
            if hasattr(gains_obj, param):
                defaults[param] = float(getattr(gains_obj, param))
    ...
    def get_parameter_bounds(self) -> ParameterBounds:
        """Get parameter bounds for all HybridTwoLayerMRAC parameters."""
    ...
    def vector_to_gains(self, parameter_vector: List[float]) -> Dict[str, Any]:
        """
        Convert parameter vector to HybridTwoLayerMRAC gain structure.
        
        Args:
            parameter_vector: Flat list of parameter values
            
        Returns:
            Dictionary of HybridTwoLayerMRAC gains
        """
    ...
    def gains_to_vector(self, gains: Dict[str, Any]) -> List[float]:
        """
        Convert HybridTwoLayerMRAC gain structure to parameter vector.
        
        Args:
            gains: Dictionary of HybridTwoLayerMRAC gains
    ...
    def get_default_gains(self) -> Dict[str, Any]:
        """Get default HybridTwoLayerMRAC gains from baseline tuning."""
        defaults = {}
